...
type Channel struct {
	...
	oversizeGossipMessageFailureTotal promshim.Counter
	oversizeGossipMessageDroppedTotal promshim.Counter
	oversizeGossipMessageSentTotal    promshim.Counter
	oversizeGossipDuration            promshim.Histogram
}
...
func NewChannel(
	...
	oversizeGossipMessageFailureTotal := promshim.With(reg).NewCounter(promshim.CounterOpts{
		Name:        "alertmanager_oversized_gossip_message_failure_total",
		Help:        "Number of oversized gossip message sends that failed.",
		ConstLabels: promshim.Labels{"key": key},
	})
	oversizeGossipMessageSentTotal := promshim.With(reg).NewCounter(promshim.CounterOpts{
		Name:        "alertmanager_oversized_gossip_message_sent_total",
		Help:        "Number of oversized gossip message sent.",
		ConstLabels: promshim.Labels{"key": key},
	})
	oversizeGossipMessageDroppedTotal := promshim.With(reg).NewCounter(promshim.CounterOpts{
	...
	oversizeGossipDuration := promshim.With(reg).NewHistogram(promshim.HistogramOpts{
