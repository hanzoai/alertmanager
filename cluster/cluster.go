...
type Peer struct {
	...
	failedReconnectionsCounter promshim.Counter
	reconnectionsCounter       promshim.Counter
	failedRefreshCounter       promshim.Counter
	refreshCounter             promshim.Counter
	peerLeaveCounter           promshim.Counter
	peerUpdateCounter          promshim.Counter
	peerJoinCounter            promshim.Counter
...
func (p *Peer) register(reg promshim.Registerer, name string) {
	peerInfo := promshim.With(reg).NewGauge(
		promshim.GaugeOpts{
			Name:        "alertmanager_cluster_peer_info",
			Help:        "A metric with a constant '1' value labeled by peer name.",
			ConstLabels: promshim.Labels{"peer": name},
		},
	)
	peerInfo.Set(1)
	promshim.With(reg).NewGaugeFunc(promshim.GaugeOpts{
	...
	p.failedReconnectionsCounter = promshim.With(reg).NewCounter(promshim.CounterOpts{
	...
	p.reconnectionsCounter = promshim.With(reg).NewCounter(promshim.CounterOpts{
		Name: "alertmanager_cluster_reconnections_total",
		Help: "A counter of the number of cluster peer reconnections.",
	})

	p.failedRefreshCounter = promshim.With(reg).NewCounter(promshim.CounterOpts{
...
func (p *Peer) AddState(key string, s State, reg promshim.Registerer) ClusterChannel {
	p.mtx.Lock()
	p.states[key] = s
	p.mtx.Unlock()
