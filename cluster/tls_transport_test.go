...
func newTLSTransport(file, address string, port int) (*TLSTransport, error) {
	...
	return NewTLSTransport(context2.Background(), promslog.NewNopLogger(), promshim.NewRegistry(), address, port, cfg)
}
...
func TestFinalAdvertiseAddr(t *testing.T) {
	...
	for _, tc := range testCases {
		...
		transport, err := NewTLSTransport(context2.Background(), logger, promshim.NewRegistry(), tc.bindAddr, tc.bindPort, tlsConf)
		require.NoError(t, err)
...
func TestWriteTo(t *testing.T) {
	...
	t1, _ := NewTLSTransport(context2.Background(), logger, promshim.NewRegistry(), "127.0.0.1", 0, tlsConf1)
	defer t1.Shutdown()

	tlsConf2 := loadTLSTransportConfig(t, "testdata/tls_config_node2.yml")
	t2, _ := NewTLSTransport(context2.Background(), logger, promshim.NewRegistry(), "127.0.0.1", 0, tlsConf2)
	defer t2.Shutdown()
...
func TestDialTimeout(t *testing.T) {
	tlsConf1 := loadTLSTransportConfig(t, "testdata/tls_config_node1.yml")
	t1, err := NewTLSTransport(context2.Background(), logger, promshim.NewRegistry(), "127.0.0.1", 0, tlsConf1)
	require.NoError(t, err)
	defer t1.Shutdown()

	tlsConf2 := loadTLSTransportConfig(t, "testdata/tls_config_node2.yml")
	t2, err := NewTLSTransport(context2.Background(), logger, promshim.NewRegistry(), "127.0.0.1", 0, tlsConf2)
	require.NoError(t, err)
	defer t2.Shutdown()
