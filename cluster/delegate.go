...
type delegate struct {
	*Peer
	...
	messagesReceived     *promshim.CounterVec
	messagesReceivedSize *promshim.CounterVec
	messagesSent         *promshim.CounterVec
	messagesSentSize     *promshim.CounterVec
	messagesPruned       promshim.Counter
	nodeAlive            *promshim.CounterVec
	nodePingDuration     *promshim.HistogramVec
	conflictsCount       promshim.Counter
}

func newDelegate(l *slog.Logger, reg promshim.Registerer, p *Peer, retransmit int) *delegate {
	bcast := &memberlist.TransmitLimitedQueue{
		NumNodes:       p.ClusterSize,
		RetransmitMult: retransmit,
	}
	messagesReceived := promshim.With(reg).NewCounterVec(promshim.CounterOpts{
		Name: "alertmanager_cluster_messages_received_total",
		Help: "Total number of cluster messages received.",
	}, []string{"msg_type"})
	messagesReceivedSize := promshim.With(reg).NewCounterVec(promshim.CounterOpts{
		Name: "alertmanager_cluster_messages_received_size_total",
		Help: "Total size of cluster messages received.",
	}, []string{"msg_type"})
	messagesSent := promshim.With(reg).NewCounterVec(promshim.CounterOpts{
		Name: "alertmanager_cluster_messages_sent_total",
		Help: "Total number of cluster messages sent.",
	}, []string{"msg_type"})
	messagesSentSize := promshim.With(reg).NewCounterVec(promshim.CounterOpts{
		Name: "alertmanager_cluster_messages_sent_size_total",
		Help: "Total size of cluster messages sent.",
	}, []string{"msg_type"})
	messagesPruned := promshim.With(reg).NewCounter(promshim.CounterOpts{
	...
	nodeAlive := promshim.With(reg).NewCounterVec(promshim.CounterOpts{
		Name: "alertmanager_cluster_alive_messages_total",
		Help: "Total number of received alive messages.",
	}, []string{"peer"},
	)
	nodePingDuration := promshim.With(reg).NewHistogramVec(promshim.HistogramOpts{
	...
	conflictsCount := promshim.With(reg).NewCounter(promshim.CounterOpts{
