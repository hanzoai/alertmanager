...
func TestAggrGroup(t *testing.T) {
	...
	ag := newAggrGroup(context.Background(), lset, route, nil, types.NewMarker(promshim.NewRegistry()), promslog.NewNopLogger())
	go ag.run(ntfy)
...
func TestDispatcher_DoMaintenance(t *testing.T) {
	...
	aggrGroup1 := newAggrGroup(ctx, labels, route, timeout, types.NewMarker(promshim.NewRegistry()), promslog.NewNopLogger())
