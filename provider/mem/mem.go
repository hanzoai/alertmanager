...
type Alerts struct {
	...
	alertsLimit             promshim.Gauge
	alertsLimitedTotal      *promshim.CounterVec
	subscriberChannelWrites *promshim.CounterVec
}
...
func (a *Alerts) registerMetrics(r promshim.Registerer) {
	r.MustRegister(&alertsCollector{alerts: a})

	a.alertsLimit = promshim.With(r).NewGauge(promshim.GaugeOpts{
	...
	a.alertsLimitedTotal = promshim.With(r).NewCounterVec(
	...
	a.subscriberChannelWrites = promshim.With(r).NewCounterVec(
		promshim.CounterOpts{
...
// alertsCollector implements promshim.Collector to collect all alert count metrics in a single pass.
type alertsCollector struct {
	alerts *Alerts
}

var alertsDesc = promshim.NewDesc(
	"alertmanager_alerts",
	"How many alerts by state.",
	[]string{"state"}, nil,
)

func (c *alertsCollector) Describe(ch chan<- *promshim.Desc) {
	ch <- alertsDesc
}

func (c *alertsCollector) Collect(ch chan<- promshim.Metric) {
	active, suppressed, unprocessed := c.alerts.countByState()

	ch <- promshim.MustNewConstMetric(alertsDesc, promshim.GaugeValue, float64(active), string(types.AlertStateActive))
	ch <- promshim.MustNewConstMetric(alertsDesc, promshim.GaugeValue, float64(suppressed), string(types.AlertStateSuppressed))
	ch <- promshim.MustNewConstMetric(alertsDesc, promshim.GaugeValue, float64(unprocessed), string(types.AlertStateUnprocessed))
}
