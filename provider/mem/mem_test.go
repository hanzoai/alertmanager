...
func TestAlertsSubscribePutStarvation(t *testing.T) {
	marker := types.NewMarker(promshim.NewRegistry())
	alerts, err := NewAlerts(context.Background(), marker, 30*time.Minute, 0, noopCallback{}, promslog.NewNopLogger(), promshim.NewRegistry(), nil)
	if err != nil {
		t.Fatal(err)
	}
...
func TestDeadLock(t *testing.T) {
	...
	alerts, err := NewAlerts(context.Background(), marker, 5*time.Millisecond, 0, noopCallback{}, promslog.NewNopLogger(), promshim.NewRegistry(), nil)
	if err != nil {
		t.Fatal(err)
	}
	alertsToInsert := []*types.Alert{}
...
func TestAlertsSubscribe(t *testing.T) {
	...
	ctx := t.Context()
	alerts, err := NewAlerts(ctx, marker, 30*time.Minute, 0, noopCallback{}, promslog.NewNopLogger(), promshim.NewRegistry(), nil)
	if err != nil {
		t.Fatal(err)
	}
...
func TestAlertsConcurrently(t *testing.T) {
	...
	a, err := NewAlerts(context.Background(), types.NewMarker(promshim.NewRegistry()), time.Millisecond, 0, callback, promslog.NewNopLogger(), nil, nil)
	require.NoError(t, err)
