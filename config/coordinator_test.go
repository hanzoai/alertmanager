...
type fakeRegisterer struct {
	registeredCollectors []promshim.Collector
}

func (r *fakeRegisterer) Register(promshim.Collector) error {
	return nil
}

func (r *fakeRegisterer) MustRegister(c ...promshim.Collector) {
	r.registeredCollectors = append(r.registeredCollectors, c...)
}

func (r *fakeRegisterer) Unregister(promshim.Collector) bool {
	return false
}
...
func TestCoordinatorNotifiesSubscribers(t *testing.T) {
	...
	c := NewCoordinator("testdata/conf.good.yml", promshim.NewRegistry(), promslog.NewNopLogger())
