...
type API struct {
	...
	requestDuration          *promshim.HistogramVec
	requestsInFlight         promshim.Gauge
	concurrencyLimitExceeded promshim.Counter
...
func New(opts Options) (*API, error) {
	...
	requestsInFlight := promshim.NewGauge(promshim.GaugeOpts{
		Name:        "alertmanager_http_requests_in_flight",
		Help:        "Current number of HTTP requests being processed.",
		ConstLabels: promshim.Labels{"method": "get"},
	})
	concurrencyLimitExceeded := promshim.NewCounter(promshim.CounterOpts{
...
func (api *API) instrumentHandler(prefix string, h http.Handler) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		promshim.InstrumentHandlerDuration(
			api.requestDuration.MustCurryWith(promshim.Labels{"handler": path}),
