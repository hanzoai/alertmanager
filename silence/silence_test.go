...
func TestSilenceGCOverTime(t *testing.T) {
	t.Run("GC does not remove active silences", func(t *testing.T) {
		s, err := New(Options{Metrics: promshim.NewRegistry()})
...
func TestQSince(t *testing.T) {
	...
	for name, c := range cases {
		t.Run(name, func(t *testing.T) {
			silences, err := New(Options{Metrics: promshim.NewRegistry()})
			require.NoError(t, err)
...
func TestSilenceExpire(t *testing.T) {
	s, err := New(Options{Metrics: promshim.NewRegistry(), Retention: time.Hour})
	require.NoError(t, err)
...
func TestSilenceExpireWithZeroRetention(t *testing.T) {
	s, err := New(Options{Metrics: promshim.NewRegistry(), Retention: 0})
	require.NoError(t, err)
...
func TestSilencer(t *testing.T) {
	ss, err := New(Options{Metrics: promshim.NewRegistry(), Retention: time.Hour})
	require.NoError(t, err)
	...
	m := types.NewMarker(promshim.NewRegistry())
