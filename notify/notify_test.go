...
func TestRetryStageWithError(t *testing.T) {
	...
	r := NewRetryStage(i, "", NewMetrics(promshim.NewRegistry(), featurecontrol.NoopFlags{}))
...
func TestMuteStage(t *testing.T) {
	...
	metrics := NewMetrics(promshim.NewRegistry(), featurecontrol.NoopFlags{})
...
func TestMuteStageWithSilences(t *testing.T) {
	silences, err := silence.New(silence.Options{Metrics: promshim.NewRegistry(), Retention: time.Hour})
	if err != nil {
		t.Fatal(err)
	}
...
func TestReceiverData_PreservationWhenNotifierDoesNotUpdate(t *testing.T) {
	...
	retryStage := NewRetryStage(integration, "test", NewMetrics(promshim.NewRegistry(), featurecontrol.NoopFlags{}))
