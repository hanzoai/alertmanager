...
import (
	...
	promslogflag "github.com/hanzoai/common/promslog/flag"
	"github.com/hanzoai/common/route"
	"github.com/hanzoai/common/version"
	"github.com/hanzoai/exporter-toolkit/web"
	webflag "github.com/hanzoai/exporter-toolkit/web/kingpinflag"
...
var (
	requestDuration = promshim.NewHistogramVec(
		promshim.HistogramOpts{
...
func instrumentHandler(handlerName string, handler http.HandlerFunc) http.HandlerFunc {
	handlerLabel := promshim.Labels{"handler": handlerName}
	return promshim.InstrumentHandlerDuration(
		requestDuration.MustCurryWith(handlerLabel),
		promshim.InstrumentHandlerResponseSize(
...
func run() int {
	...
	promshim.MustRegister(promshim.NewCollector("alertmanager"))
	...
	notificationLogOpts := nflog.Options{
		...
		Metrics:      promshim.DefaultRegisterer,
	}
	...
	if peer != nil {
		c := peer.AddState("nfl", notificationLog, promshim.DefaultRegisterer)
	...
	marker := types.NewMarker(promshim.DefaultRegisterer)
	...
	if peer != nil {
		c := peer.AddState("sil", silences, promshim.DefaultRegisterer)
	...
	api, err := api.New(api.Options{
		...
		Registry:        promshim.DefaultRegisterer,
	...
	dispMetrics := dispatch.NewDispatcherMetrics(false, promshim.DefaultRegisterer)
	pipelineBuilder := notify.NewPipelineBuilder(promshim.DefaultRegisterer, ff)
